...
function cekform(mode){
  if($('#'+mode+'_'+'program_id').val()==""){
    alert('Program is empty.');
    $('#'+mode+'_'+'program_id').focus();
    return false;
  }
...
function show_modal(mode, data_arr) {
  if(mode == 'add'){
    ...
    $('#edit_program_id').val(data_arr['program_id']);
    ...
    if(mode == 'edit'){
      ...
      //$('#div_edit_files').prop('src', '{{ asset('/') }}files/topic/'+data_arr['files']);
      $('#data_edit_files').html('');
      $.each(data_arr['files'].split(','), function(key, value){
        $('#data_edit_files').append('<a href="{{ asset('/') }}files/topic/'+value+'" target="_blank">'+value+'</a>');
        //console.log(value);
      });
      //$('#data_edit_files').html('background-files', 'url({{ asset('/') }}files/topic/'+data_arr['files']+')');
      $('#edit_modal_name').html('View {{ $title }}');
      $('#edit_modal_footer').hide();
    }
    $('#edit_id').prop('disabled', var_dis);
    $('#edit_program_id').prop('disabled', var_dis);
        ...
        ?>
          <tr>
              ...
              ?>})" class="btn bg-orange btn-circle btn-xs" title="Edit Data"><i class="fa fa-edit"></i></button>
                <a href="{{ url('/') }}/backend/topic/delete/{{ $row->id }}" class="btn bg-maroon btn-circle btn-xs" title="Hapus Data" onclick="return confirm('Are you sure want to DELETE topic {{ $row->name }}?')"><i class="fa fa-trash"></i></a>
              </td>
          </tr>
        @endforeach
        </tbody>
...
<div class="modal fade" id="add_modal">
  <div class="modal-dialog modal-lg">
    <form class="modal-content" name="add_frm" id="add_frm" method="post" enctype="multipart/form-data" name="frm" action="{{ url('/') }}/backend/topic/add/post">
      {{ csrf_field() }}
      ...
      <div class="modal-body">
        <div class="form-horizontal">
          <div class="form-group row">
            ...
            <div class="col-sm-8">
              <select class="form-control select2bs4" name="program_id" id="add_program_id" data-placeholder="Pilih Program..." style="width: 100%;">
                <option value=""></option>
...
<div class="modal fade" id="edit_modal">
  <div class="modal-dialog modal-lg">
    <form class="modal-content" name="edit_frm" id="edit_frm" method="post" enctype="multipart/form-data" name="frm" action="{{ url('/') }}/backend/topic/edit/post">
      ...
      <div class="modal-body">
        <div class="form-horizontal">
          <div class="form-group row">
            <label class="col-sm-4 col-form-label">Program</label>
            <div class="col-sm-8">
              <select class="form-control select2bs4" name="program_id" id="edit_program_id" data-placeholder="Pilih Program..." style="width: 100%;">
                <option value=""></option>
                @foreach($program as $row)
                <option value="{{ $row->id }}">{{ $row->name }}</option>
                @endforeach
